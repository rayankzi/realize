...
3. Based on the title and topic, generate a short, descriptive, kebab-case base name (3-5 words max, e.g. `data-science-roadmap`, `quick-pasta-recipe`, `productivity-tips`). Append the date suffix so the final filename is `<base>-<M>-<D>.md` (e.g. `data-science-roadmap-5-26.md`).
